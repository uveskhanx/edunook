...
import { 
  ...
  ShieldAlert, IndianRupee, Rocket, Globe, Eye, Plus, Upload, FileVideo, Globe2, Layers, Info, X
} from 'lucide-react';
...
import { createCourseOrder, loadRazorpayCheckout, RazorpayPaymentResponse, verifyCoursePayment } from '@/lib/razorpay-client';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function CourseClient({ slug }: { slug: string }) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'reviews' | 'management'>('overview');
  const [newChapterData, setNewChapterData] = useState({ title: '', type: 'video' as 'video' | 'link' | 'quiz', url: '', isFreeDemo: false });
  ...
  const loadUserTests = async () => {
    ...
    try {
      const tests = await DbService.getUserTests(currentUser.id);
      setUserTests(tests);
    } catch (err) {
  ...
  const handleAddChapter = async (e?: React.FormEvent) => {
    ...
    if (!newChapterData.title.trim()) return toast.error('Module title is required');
    if (newChapterData.type === 'video' && !selectedVideo) return toast.error('Please select a video file');
    if ((newChapterData.type === 'link' || newChapterData.type === 'quiz') && !newChapterData.url) return toast.error('Resource link is required');

    setAddingChapter(true);
    setUploadProgress(0);
    try {
      let finalUrl = newChapterData.url;
      
      if (newChapterData.type === 'video' && selectedVideo) {
        toast.loading('Starting secure video sync...', { id: 'v-sync' });
        // Using a interval to simulate progress since DbService.uploadVideo is a single promise
        const progInterval = setInterval(() => {
           setUploadProgress(prev => {
              if (prev >= 95) { clearInterval(progInterval); return 95; }
              return prev + 5;
           });
        }, 800);

        finalUrl = await DbService.uploadVideo(currentUser.id, course.id, selectedVideo, chapters.length);
        clearInterval(progInterval);
        setUploadProgress(100);
        toast.success('Video synced to Cloudinary!', { id: 'v-sync' });
      }

      const position = chapters.length + 1;
      const chapterId = await DbService.addChapter(course.id, {
        title: newChapterData.title,
        type: newChapterData.type,
        videoUrl: newChapterData.type === 'video' ? finalUrl : '',
        pageUrl: newChapterData.type === 'link' ? finalUrl : '',
        quizUrl: newChapterData.type === 'quiz' ? finalUrl : '',
        isFreeDemo: newChapterData.isFreeDemo,
        position,
      });
      ...
      setNewChapterData({ title: '', type: 'video', url: '', isFreeDemo: false });
  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      const loadingToast = toast.loading('Creating secure Razorpay order...');
      const [order, isLoaded] = await Promise.all([
        createCourseOrder(course.id, currentUser.id),
      ...
      toast.loading('Opening secure Razorpay checkout...', { id: loadingToast });
      ...
      await new Promise<void>((resolve, reject) => {
        const razorpay = new RazorpayCheckout({
          ...
          handler: async (response: RazorpayPaymentResponse) => {
            try {
              toast.loading('Verifying payment securely...', { id: loadingToast });
                         ...
                         <div className="relative z-10 flex flex-col items-center gap-5 md:gap-7 p-6 md:p-10 bg-black/70 backdrop-blur-3xl rounded-[2rem] md:rounded-[2.5rem] border border-white/10 shadow-2xl max-w-lg mx-4 text-center">
                           ...
                           <div className="space-y-2 md:space-y-3">
                            ...
                            <p className="text-[10px] font-bold text-white/35 leading-relaxed">
                              Secure Razorpay checkout ready.
                            </p>
                         </div>
                     ) : (
                        <>
          ...
          <div className={`p-4 md:p-10 lg:p-16 ${theaterMode ? 'max-w-[1800px]' : 'max-w-[1600px]'} mx-auto w-full space-y-12 md:space-y-24 pb-40`}>
             ...
             <div className="space-y-6 md:space-y-12">
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row md:items-center justify-between gap-8 md:gap-12 p-6 md:p-10 bg-white/[0.02] border border-white/5 rounded-[2rem] md:rounded-[3rem] backdrop-blur-3xl relative overflow-hidden group">
                    ...
                    <Link href={`/${course.profiles?.username || ''}`} className="flex items-center gap-5 md:gap-6 relative z-10">
             ...
             <div className="flex flex-col gap-16 md:gap-24">
                ...
                <div className="flex items-center gap-8 md:gap-14 border-b border-white/5 pb-2">
                   {(['overview', 'reviews', ...(isOwner ? ['management'] : [])] as const).map((tab) => (
                      <button
                        key={tab}
                        onClick={() => setActiveTab(tab as 'overview' | 'reviews' | 'management')}
                      ...
                      >
                         <div className="flex items-center gap-3">
                            ...
                            {tab === 'management' && <Settings className="w-4 h-4" />}
                            {tab}
                         </div>
                         {activeTab === tab && (
                ...
                <AnimatePresence mode="wait">
                    {activeTab === 'overview' ? (
                      ...
                      <motion.div key="mgmt" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-10">
                        
                        {/* Management Header */}
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6">
                           <div className="space-y-1">
                              <h2 className="text-3xl font-black text-white uppercase tracking-tighter">Course Studio</h2>
                              <p className="text-muted-foreground text-xs font-medium uppercase tracking-[0.2em] opacity-50">Curriculum & Assets</p>
                           </div>
                           <div className="flex items-center gap-3">
                              <button 
                                onClick={() => { setIsAddingModule(true); loadUserTests(); }}
                                className="px-6 py-3 bg-primary text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-2xl shadow-primary/30 flex items-center gap-3 hover:scale-105 active:scale-95 transition-all"
                              >
                        ...
                        {isAddingModule ? (
                          <motion.div 
                            ...
                            className="bg-white/[0.03] border border-white/10 rounded-[3rem] p-10 space-y-12 relative overflow-hidden"
                          >
                             <div className="absolute top-6 right-6">
                                <button onClick={() => setIsAddingModule(false)} className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-all">
                                   <X className="w-5 h-5" />
                                </button>
                             </div>
                             ...
                             <div className="max-w-4xl mx-auto space-y-10">
                                <div className="text-center space-y-2">
                                   <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Deploy Content</h3>
                                   <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest">Step 01: Select Resource Type</p>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                   {[
                                      { id: 'video', label: 'Video Class', icon: FileVideo, color: 'bg-blue-500', desc: 'MP4, MOV, WEBM' },
                                      { id: 'link', label: 'Web Resource', icon: Globe2, color: 'bg-purple-500', desc: 'External tools/links' },
                                      { id: 'quiz', label: 'Assessment', icon: FileQuestion, color: 'bg-amber-500', desc: 'Select from tests' }
                                   ].map(type => (
                                      <button
                                         key={type.id}
                                         onClick={() => { setNewChapterData(prev => ({ ...prev, type: type.id as any })); setSelectedVideo(null); }}
                                         className={`p-6 rounded-[2.5rem] border transition-all text-left group relative overflow-hidden ${newChapterData.type === type.id ? 'bg-white border-white' : 'bg-white/[0.02] border-white/5 hover:border-white/20'}`}
                                      >
                                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 transition-all ${newChapterData.type === type.id ? type.color + ' text-white' : 'bg-white/5 text-white/40'}`}>
                                            <type.icon className="w-6 h-6" />
                                         </div>
                                         <p className={`text-[11px] font-black uppercase tracking-widest ${newChapterData.type === type.id ? 'text-black' : 'text-white'}`}>{type.label}</p>
                                         <p className={`text-[9px] font-medium mt-1 ${newChapterData.type === type.id ? 'text-black/60' : 'text-muted-foreground/60'}`}>{type.desc}</p>
                                         {newChapterData.type === type.id && (
                                            <motion.div layoutId="sel-badge" className="absolute top-4 right-4 w-2 h-2 rounded-full bg-primary" />
                                         )}
                                      </button>
                                   ))}
                                </div>
                                ...
                                <div className="space-y-8 pt-6 border-t border-white/5">
                                   <div className="space-y-4">
                                      <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Module Title</label>
                                      <input 
                                         required
                                         ...
                                         className="w-full bg-black/40 border border-white/5 rounded-[1.5rem] p-5 text-base font-bold text-white focus:ring-2 focus:ring-primary/40 outline-none"
                                      />
                                   </div>

                                   {newChapterData.type === 'video' ? (
                                      <div className="space-y-4">
                                         <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Video Source</label>
                                         <input type="file" accept="video/*" id="v-up" className="hidden" onChange={e => setSelectedVideo(e.target.files?.[0] || null)} />
                                         <label htmlFor="v-up" className={`w-full flex flex-col items-center justify-center gap-4 py-12 border-2 border-dashed rounded-[2.5rem] cursor-pointer transition-all ${selectedVideo ? 'bg-primary/5 border-primary/40' : 'bg-black/40 border-white/5 hover:border-white/20'}`}>
                                            {selectedVideo ? (
                                               <><FileVideo className="w-10 h-10 text-primary animate-bounce" /><div className="text-center"><p className="text-sm font-black text-white">{selectedVideo.name}</p><p className="text-[9px] font-black text-primary uppercase mt-1">Ready to sync</p></div></>
                                            ) : (
                                               <><Upload className="w-10 h-10 text-white/20" /><div className="text-center"><p className="text-sm font-black text-white">Upload Video Asset</p><p className="text-[9px] font-black text-muted-foreground uppercase mt-1">Direct Cloudinary Sync</p></div></>
                                            )}
                                         </label>
                                      </div>
                                   ) : newChapterData.type === 'quiz' ? (
                                      <div className="space-y-4">
                                         <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Select from Published Tests</label>
                                         <div className="space-y-2">
                                            {loadingTests ? (
                                               <div className="py-8 flex items-center justify-center gap-3 text-muted-foreground"><Loader2 className="w-4 h-4 animate-spin" /> Fetching your quizes...</div>
                                            ) : userTests.length === 0 ? (
                                               <div className="py-8 text-center bg-black/40 border border-white/5 rounded-2xl text-muted-foreground text-xs">No tests found. Create a test first.</div>
                                            ) : (
                                               <div className="grid grid-cols-1 gap-2 max-h-[300px] overflow-y-auto pr-2 no-scrollbar">
                                                  {userTests.map(t => (
                                                     <button 
                                                        key={t.id} 
                                                        type="button"
                                                        onClick={() => setNewChapterData(prev => ({ ...prev, url: `/test/${t.slug || t.id}` }))}
                                                        className={`p-4 rounded-2xl border text-left flex items-center justify-between transition-all ${newChapterData.url.includes(t.id) || newChapterData.url.includes(t.slug) ? 'bg-primary border-primary text-white' : 'bg-black/40 border-white/5 text-muted-foreground hover:bg-white/5'}`}
                                                     >
                                                        <div className="flex items-center gap-3">
                                                           <div className="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center"><FileQuestion className="w-4 h-4" /></div>
                                                           <span className="text-[11px] font-black uppercase tracking-widest">{t.title}</span>
                                                        </div>
                                                        {newChapterData.url.includes(t.id) && <CheckCircle className="w-4 h-4 text-white" />}
                                                     </button>
                                                  ))}
                                               </div>
                                            )}
                                      ...
                                      <div className="space-y-4">
                                         <label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest ml-1">Resource Link</label>
                                         <input 
                                            required
                                            value={newChapterData.url || ''}
                                            onChange={e => setNewChapterData(prev => ({ ...prev, url: e.target.value }))}
                                            placeholder="https://..."
                                            className="w-full bg-black/40 border border-white/5 rounded-[1.5rem] p-5 text-base font-bold text-white focus:ring-2 focus:ring-primary/40 outline-none"
                                         />
                                      </div>
                                   ...
                                   <div className="flex items-center gap-4 bg-white/5 p-6 rounded-[2rem] border border-white/5">
                                      <div className="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary"><Eye className="w-6 h-6" /></div>
                                      <div className="flex-1">
                                         <p className="text-[10px] font-black text-white uppercase tracking-widest">Free Preview Mode</p>
                                         <p className="text-[8px] font-medium text-muted-foreground">Allows users to watch without enrollment</p>
                                      </div>
                                      <div className="relative inline-flex items-center cursor-pointer">
                                         <input type="checkbox" checked={newChapterData.isFreeDemo} onChange={e => setNewChapterData(prev => ({ ...prev, isFreeDemo: e.target.checked }))} className="sr-only peer" />
                                         <div className="w-12 h-7 bg-white/10 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:rounded-full after:h-[1.3rem] after:w-[1.3rem] after:transition-all peer-checked:bg-primary"></div>
                                      </div>
                                   </div>

                                   {uploadProgress > 0 && (
                                      <div className="space-y-2">
                                         <div className="flex justify-between text-[10px] font-black uppercase text-primary"><span>Cloudinary Sync in Progress</span><span>{uploadProgress}%</span></div>
                                         <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${uploadProgress}%` }} className="h-full bg-primary" /></div>
                                      </div>
                                   )}
                                   ...
                                   <div className="flex gap-4">
                                      <button 
                                         ...
                                         className="flex-1 py-5 bg-white/5 text-white rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest hover:bg-white/10 transition-all"
                                      >
                                         Cancel
                                      </button>
                                      <button 
                                         onClick={handleAddChapter}
                                         disabled={addingChapter || (newChapterData.type === 'video' && !selectedVideo)}
                                         className="flex-[2] py-5 bg-white text-black rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest shadow-2xl hover:scale-[1.02] active:scale-95 transition-all disabled:opacity-30 disabled:scale-100"
                                      >
                                         {addingChapter ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : "Finalize & Deploy"}
                                      </button>
                                   </div>
                                </div>
                             </div>
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                            {/* Simple Curriculum View */}
                            <div className="lg:col-span-8 space-y-6">
                               <div className="space-y-4 max-h-[800px] overflow-y-auto pr-2 no-scrollbar">
                                 {chapters.map((ch, index) => (
                                   <div 
                                     key={ch.id} 
                                     className="group p-6 bg-white/[0.02] border border-white/5 hover:border-white/20 rounded-[2rem] flex items-center justify-between transition-all"
                                   >
                                     <div className="flex items-center gap-6">
                                        <div className="w-12 h-12 rounded-2xl bg-black/40 flex items-center justify-center border border-white/5 text-[11px] font-black text-muted-foreground group-hover:text-primary transition-colors">
                                           {index + 1}
                                        </div>
                                        <div className="space-y-1">
                                          <p className="text-base font-black text-white">{ch.title}</p>
                                          <div className="flex items-center gap-3">
                                             <span className="text-[9px] font-black text-primary uppercase tracking-widest flex items-center gap-1.5">
                                                {ch.type === 'video' ? <FileVideo className="w-3.5 h-3.5" /> : ch.type === 'quiz' ? <FileQuestion className="w-3.5 h-3.5" /> : <Globe2 className="w-3.5 h-3.5" />}
                                                {ch.type}
                                             </span>
                                             {ch.isFreeDemo && <span className="text-[7px] font-black text-emerald-500 uppercase tracking-widest px-1.5 py-0.5 bg-emerald-500/10 rounded-full">Preview Enabled</span>}
                                          </div>
                                        </div>
                                     </div>
                                     <button 
                                 ...
                                 {chapters.length === 0 && (
                                   <div className="py-32 text-center border-2 border-dashed border-white/5 rounded-[3rem] opacity-20">
                                      ...
                                      <p className="text-[10px] font-black uppercase tracking-[0.4em]">Ready for content</p>
                                   </div>
                                 )}
                               </div>
                            </div>
                            ...
                            <div className="lg:col-span-4 space-y-8">
                               <div className="p-8 bg-primary/5 border border-primary/20 rounded-[2.5rem] space-y-6">
                                  <div className="w-12 h-12 rounded-2xl bg-primary/20 flex items-center justify-center text-primary"><Zap className="w-6 h-6" /></div>
                                  <div className="space-y-2">
                                     <h3 className="text-lg font-black text-white uppercase tracking-tighter">Instant Curriculum</h3>
                                     <p className="text-xs text-muted-foreground/60 leading-relaxed">Expand your course instantly. New modules appear for all students as soon as they are deployed.</p>
                                  </div>
                               </div>

                               <button 
                                 onClick={() => { setIsAddingModule(true); loadUserTests(); }}
                                 className="w-full py-8 bg-white text-black rounded-[2.5rem] flex flex-col items-center justify-center gap-4 hover:scale-[1.02] active:scale-95 transition-all shadow-3xl"
                               >
                                  <div className="w-12 h-12 rounded-full bg-black text-white flex items-center justify-center"><Plus className="w-6 h-6" /></div>
                                  <span className="text-[11px] font-black uppercase tracking-[0.3em]">Add Module</span>
                               </button>
                            </div>
                          </div>
                        )}
                           ...
                           {reviews.length === 0 ? (
                              ...
                              reviews.map((r, i) => (
                                 <motion.div key={r.id || `review-${i}`} layout className="p-5 md:p-8 bg-white/5 border border-white/5 rounded-[1.5rem] md:rounded-[3rem] hover:bg-white/[0.08] transition-all group relative">
                                    ...
                                    <p className="mt-5 md:mt-8 text-[13px] md:text-[15px] text-white/70 leading-relaxed font-medium italic">
