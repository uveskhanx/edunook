...
export default function ExploreClient() {
  ...
  const queryTab = (searchParams.get('tab') as 'trending' | 'creators') || 'trending';
  ...
  const [activeTab, setActiveTab] = useState<'trending' | 'creators'>(queryTab);
  const [activeCategory, setActiveCategory] = useState(queryCategory);

  useEffect(() => {
    setActiveTab(queryTab);
    setActiveCategory(queryCategory);
    if(queryQ !== searchQuery) {
        setSearchQuery(queryQ);
    }
  }, [queryTab, queryCategory, queryQ]);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-10">
              <div className="space-y-2">
                ...
                <p className="text-xs md:text-sm text-muted-foreground font-medium max-w-md opacity-60">
                  Explore published courses and creators from the EduNook community.
                </p>
              </div>
              ...
              <div className="w-full md:w-[400px] relative group">
                ...
                <div className="relative flex items-center bg-card/50 backdrop-blur-xl border border-border rounded-2xl px-5 py-4 focus-within:border-primary/50 transition-all shadow-2xl">
                  ...
                  />
                  {searchQuery && (
                    <button onClick={() => handleSearchChange('')} className="p-1 hover:bg-white/5 rounded-full text-muted-foreground transition-colors">
                      <X className="w-4 h-4" />
                    </button>
                  )}
                </div>
                          ...
                          >
                            ...
                            <div className="mt-6 pt-6 border-t border-white/5 flex items-center justify-center gap-4">
                              <div className="text-center">
                                ...
                                <p className="text-[7px] font-black text-muted-foreground uppercase tracking-widest opacity-40">Followers</p>
                              </div>
                            </div>
