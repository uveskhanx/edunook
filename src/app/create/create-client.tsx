...
export default function CreateClient() {
      ...
      <Layout>
        <div className="flex flex-col items-center justify-center py-32 px-4 text-center">
          <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="p-12 bg-card/30 backdrop-blur-3xl border border-border rounded-[3rem] shadow-2xl max-w-md">
            ...
            <Link href="/login" className="block w-full py-5 bg-primary text-white rounded-2xl font-black shadow-2xl shadow-primary/20 hover:scale-105 transition-all">
              Ignite Your Journey
            </Link>
          </motion.div>
        </div>
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const missingChapterContent = chapters.find((chapter) => {
      if (!chapter.title.trim()) return true;
      if (chapter.type === 'video') return !chapter.file;
      if (chapter.type === 'link') return !chapter.pageUrl?.trim();
      if (chapter.type === 'quiz') return !chapter.quizUrl?.trim();
      return true;
    });
    if (missingChapterContent) {
      toast.error('Every chapter needs a title and its required content.');
      setStep(2);
      return;
    }
    ...
    if (isPaid && finalPrice <= 0) {
      toast.error('Paid courses need a valid price greater than 0.');
      setStep(1);
      return;
    }

    if (isPaid && hasExpiry && (!expiresInDays || parseInt(expiresInDays, 10) <= 0)) {
      toast.error('Limited access needs a positive duration in days.');
      setStep(3);
      return;
    }
    ...
    try {
      ...
      if (isPaid) {
        const paymentSettings = await DbService.getTeacherPaymentSettings(user.id);
        if (!paymentSettings.razorpay_account_id) {
          toast.error('Add your Razorpay linked account ID in Settings before publishing a paid course.');
          setStep(1);
          return;
        }
      }
      ...
      const createdCourse = await DbService.createCourse(user.id, {
        title,
        description,
        price: finalPrice,
        category,
      ...
      for (let i = 0; i < chapters.length; i++) {
        ...
        if (ch.type === 'video' && ch.file) {
          ...
          videoUrl = await DbService.uploadVideo(user.id, createdCourse.id, ch.file, i);
          setUploadProgress(prev => ({ ...prev, [ch.id]: 100 }));
        }

        await DbService.addChapter(createdCourse.id, {
      ...
      router.push(`/course/${createdCourse.slug}`);
    } catch (err: any) {
      console.error(err);
                              ...
                              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-3 overflow-hidden">
                                ...
                                <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center gap-3">
                                   ...
                                   <p className="text-[10px] font-black text-emerald-500/80 uppercase tracking-widest leading-tight">
                                     Estimated Revenue: <span className="text-foreground">Rs. {(parseFloat(price || '0') * 0.65).toFixed(0)}</span> per student
                                   </p>
                                </div>
                              </motion.div>
                            )}
