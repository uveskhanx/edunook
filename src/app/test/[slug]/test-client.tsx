...
const formatDuration = (s: number) => {
  ...
  const rs = Math.floor((totalCentiseconds % 6000) / 100);
  const cs = totalCentiseconds % 100;
  return m > 0
    ? `${m}m ${rs.toString().padStart(2, '0')}.${cs.toString().padStart(2, '0')}s`
    : `${rs}.${cs.toString().padStart(2, '0')}s`;
};
...
export default function TestViewClient() {
  ...
  const persistedTimeTakenSecond = Math.floor(timeTaken);
    ...
    async function loadData() {
      ...
      try {
        ...
        if (t) {
          ...
          if (user?.id) {
            const submittedAttempt = await DbService.getBestTestAttemptForUser(user.id, t.id);
            if (submittedAttempt) {
              setFinished(true);
              setStarted(false);
              setScore(Math.round((submittedAttempt.score / (submittedAttempt.total || q.length)) * 100));
              setAnswers(submittedAttempt.answers || {});
              setHintsUsed(submittedAttempt.hintsUsed || []);
              setTimeTaken(submittedAttempt.timeTaken ?? (submittedAttempt.timeTakenCentiseconds ? submittedAttempt.timeTakenCentiseconds / 100 : 0));
              localStorage.setItem(storageKey, JSON.stringify({
                testId: t.id,
                answers: submittedAttempt.answers || {},
                hintsUsed: submittedAttempt.hintsUsed || [],
                timeTaken: submittedAttempt.timeTaken ?? (submittedAttempt.timeTakenCentiseconds ? submittedAttempt.timeTakenCentiseconds / 100 : 0),
                timeTakenCentiseconds: submittedAttempt.timeTakenCentiseconds,
                finished: true
              }));
              return;
            }
          ...
          if (saved) {
            ...
            if (parsed.testId === t.id) {
              if (parsed.finished) {
                ...
                setTimeTaken(parsed.timeTaken ?? (parsed.timeTakenCentiseconds ? parsed.timeTakenCentiseconds / 100 : 0));
              } else {
  ...
  useEffect(() => {
    if (started && !finished && test) {
      localStorage.setItem(storageKey, JSON.stringify({
        ...
        currentIdx,
        timeTaken,
        timeTakenCentiseconds: Math.round(timeTaken * 100),
        finished: false
      }));
    }
  }, [answers, hintsUsed, currentIdx, persistedTimeTakenSecond, started, finished, test]);

  // Timer
  useEffect(() => {
    if (started && !finished && test) {
      if (startedAtRef.current === null) {
        startedAtRef.current = Date.now() - Math.round(timeTaken * 1000);
      }
      timerRef.current = setInterval(() => {
        if (startedAtRef.current === null) return;
        setTimeTaken(Number(((Date.now() - startedAtRef.current) / 1000).toFixed(2)));
      }, 100);
    }
  ...
  const handleFinish = async () => {
    ...
    const finalTimeTaken = startedAtRef.current === null
      ? timeTaken
      : Number(((Date.now() - startedAtRef.current) / 1000).toFixed(2));
    const finalTimeTakenCentiseconds = Math.round(finalTimeTaken * 100);
    ...
    try {
      localStorage.setItem(storageKey, JSON.stringify({ 
        ...
        timeTakenCentiseconds: finalTimeTakenCentiseconds,
        finished: true 
      }));
      setFinished(true);
      ...
      await DbService.saveLeaderboardEntry(test.slug || test.id, user.id, {
        score: correct,
  ...
  if (!test || questions.length === 0) return (
    <Layout hideNavigation={true}>
      <div className={`min-h-screen ${theme.bg} flex flex-col items-center justify-center p-6 text-center`}>
        ...
        <Link href="/tests" className="px-10 py-5 bg-white text-black font-black rounded-2xl hover:scale-105 transition-all">Return to Assessment Hub</Link>
      </div>
    </Layout>
  );
           ...
           <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl w-full relative z-10 text-center space-y-16">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-8 max-w-3xl mx-auto">
                 {[
                   ...
                   { label: 'Result Mode', val: test.resultAnnounceAt ? 'Scheduled' : 'Instant', icon: Award, color: theme.accent },
                 ].map((stat, i) => (
              ...
              <div className="flex flex-col md:flex-row items-center justify-center gap-6 pt-10">
                 {timeTaken > 0 ? (
                   <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
                     <button onClick={startQuiz} className={`w-full md:w-auto px-12 py-6 ${theme.primary} text-white font-black text-[10px] uppercase tracking-widest rounded-2xl hover:scale-105 transition-all shadow-xl flex items-center justify-center gap-3`}>
                        Resume Mission ({formatDuration(timeTaken)}) <RefreshCcw className="w-4 h-4" />
                     </button>
                     <button 
                   ...
                   <button onClick={startQuiz} className="w-full md:w-auto px-16 py-7 bg-white text-black font-black text-[10px] uppercase tracking-[0.3em] rounded-3xl hover:scale-105 active:scale-95 transition-all shadow-2xl flex items-center justify-center gap-4 group">
                      Enter Assessment Arena <ArrowRight className="w-5 h-5 group-hover:translate-x-2 transition-transform" />
                   </button>
                 )}
                 <Link href="/tests" className="text-white/20 hover:text-white font-black text-[10px] uppercase tracking-[0.4em] transition-colors">Abort Assessment</Link>
              </div>
           </motion.div>
        </div>
      </Layout>
         ...
         <div className={`min-h-screen ${theme.bg} flex flex-col items-center p-6 relative overflow-y-auto custom-scrollbar pt-24 pb-32`}>
           <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="max-w-4xl w-full bg-white/[0.03] border border-white/10 rounded-[4rem] p-10 md:p-20 text-center space-y-20 relative z-10 mb-20 shadow-2xl">
              <div className="space-y-12">
                 ...
                 <div className="space-y-6">
                    ...
                    <p className="text-lg md:text-xl text-white/40 font-medium max-w-xl mx-auto leading-relaxed opacity-60 italic">
                      {isPassing ? 'You passed this assessment. Review the leaderboard or retake it to improve your time.' : 'You did not pass yet. Review the syllabus and try again when ready.'}
                    </p>
                 </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { label: 'Time Invested', val: formatDuration(timeTaken), icon: Clock, color: theme.accent },
                    ...
                    { label: 'Leaderboard', val: 'View', icon: Trophy, color: theme.accent, action: () => setSelectedRankTest(test) }
                  ].map((stat, i) => (
                    <motion.button 
                      key={i}
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-10">
                 <Link href="/tests" className="w-full sm:w-auto px-20 py-7 bg-white text-black font-black text-[10px] uppercase tracking-[0.3em] rounded-2xl shadow-3xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-4">Return to Assessment Hub <ArrowRight className="w-5 h-5" /></Link>
              </div>
           </motion.div>
      ...
      <Layout hideNavigation={true}>
        <div className={`min-h-screen ${theme.bg} flex items-center justify-center p-6 text-center`}>
          <div className="max-w-md space-y-4">
            ...
            <h1 className="text-2xl font-black text-white uppercase tracking-tight">No Questions Available</h1>
            <p className="text-sm text-white/50 font-medium">This assessment cannot be started until the creator adds at least one question.</p>
            <Link href="/tests" className="inline-flex px-6 py-3 rounded-xl bg-white text-black text-xs font-black uppercase tracking-widest">
              Back to Assessments
            </Link>
          </div>
        </div>
      ...
      <div className={`min-h-screen ${theme.bg} relative flex flex-col p-4 md:p-8 overflow-hidden select-none transition-colors duration-1000`}>
        <div className="max-w-7xl mx-auto w-full flex flex-col flex-1 relative z-10">
          <header className="flex flex-col md:flex-row items-center justify-between gap-8 mb-12 md:mb-20 pt-2">
            <Link href="/tests" className="w-full md:w-auto group flex items-center justify-center gap-3 px-6 py-4 bg-white/5 border border-white/10 text-white/40 hover:text-white rounded-2xl transition-all font-black text-[10px] uppercase tracking-widest">
                            ...
                            <AnimatePresence>
                               {showHint && (
                                  ...
                                  >
                                     ...
                                     <p className="text-xs font-black text-amber-200 leading-relaxed italic uppercase tracking-tighter">&ldquo;{currentQ.hint}&rdquo;</p>
                                  </motion.div>
                               )}
                            </AnimatePresence>
                            <button 
