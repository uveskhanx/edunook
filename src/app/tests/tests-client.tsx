...
export default function TestsClient() {
  ...
  const startQuiz = (test: TestRow) => {
    ...
    router.push(`/test/${test.slug || test.id}`);
  };
          ...
          <div className="space-y-8 md:space-y-12">
            ...
            >
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-10 border-l-4 border-primary/20 pl-6 md:pl-12">
                <p className="text-lg md:text-3xl text-white/50 font-medium max-w-2xl leading-relaxed">
                   Create and take timed quizzes with clear schedules, instant feedback, and transparent standings.
                </p>
                {user && (
                  <button 
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6">
               {[
                 ...
                 { label: 'Quiz Attempts', val: globalStats.enrollments, icon: Smile, color: 'text-blue-400' },
                 { label: 'Perfect Scores', val: globalStats.trophies, icon: Trophy, color: 'text-amber-400' },
               ].map((stat, i) => (
                 <motion.div 
                   key={i}
                    ...
                    <div className="relative z-10 flex flex-row md:items-center gap-5 md:gap-8 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0 space-y-1.5 md:space-y-4">
                         <div className="flex flex-wrap items-center gap-2 md:gap-4">
                            <span className="text-[8px] md:text-[10px] font-black text-primary uppercase tracking-[0.2em] md:tracking-[0.3em] flex items-center gap-1.5">
                               <User className="w-3 md:w-3.5 h-3 md:h-3.5" /> {test.profiles?.fullName?.split(' ')[0] || 'Creator'}
                            </span>
                      ...
                      <div className="flex items-center gap-2 md:gap-4 shrink-0 self-center md:self-auto">
                        ...
                        >
                          ...
                          ) : status === 'upcoming' ? 'Locked' : 'Closed'} <ArrowRight className="w-3.5 h-3.5 md:w-5 md:h-5" />
                        </button>
                      </div>
                    </div>
                  </motion.div>
...
function CinematicQuizCreator({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  const handleCreate = async () => {
    ...
    if (!quizData.title.trim()) {
      toast.error('Add an assessment title first.');
      return;
    }
    if (quizData.timeLimit < 1) {
      toast.error('Assessment duration must be at least 1 minute.');
      return;
    }
    if (quizData.activeStartAt && quizData.expiresAt && new Date(quizData.activeStartAt) >= new Date(quizData.expiresAt)) {
      toast.error('The deadline must be after the availability start.');
      return;
    }
            ...
            {step === 1 ? (
              <div className="space-y-12 md:space-y-16">
                 ...
                 {/* Visual Identity (Themes) */}
                  <div className="space-y-6">
                    <Label className="text-[10px] font-black uppercase tracking-widest text-white/30 ml-2">Assessment Visual Identity</Label>
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      {[
                        ...
                        <button
                          key={t.id}
                          onClick={() => setQuizData({ ...quizData, theme: t.id as QuizTheme })}
